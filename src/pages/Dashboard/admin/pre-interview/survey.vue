<template>
  <div>
    ...
    <div :class="{fade : loading }">
     <h5>Pre-Interview Survey : {{ survey.name }}</h5>
     <hr />
     
      <h6>
        <img class="candidate-img-survey" height="75" width="75" :src="candidate.photo" />
        {{ candidate['First Name'] }} {{ candidate['Last Name'] }} Current Score: {{ score }}
      </h6>
...
export default {
  ...
  watch: {
    ...
    computed: {
      readySurvey() {
        // console.log(this.candidate.comments)
        let x
      ...
      candidateUpdate(){
        ...
        candidate.preinterview_score = (this.score + candidate.preinterview_score)

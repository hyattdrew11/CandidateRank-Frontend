    ...
    <div v-if="currentTerm && !loading" id="evaluator-schedule-container">
      ...
      <b-container fluid>
        <b-row>
          ...
          <b-col
            md="4"
            v-for="(x, index) in currentTerm.dates"
            v-bind:key="index"
            class="mb-2"
          >
            <b-card header="">
...
export default {
  ...
  data() {
    return {
      ...
      success: false,
      successMsg: `Your interview date has been accepted. Please check your email 
      inbox for calendar invites or return to this page to review your schedule below. 
      If you are rescheduling please choose a date above.`,
      slots: [],
    };
  },
  components: {},
  ...
  methods: {
    ...
    chooseDate(x) {
      ...
      axios
        ...
        .catch(function (e) {
          alert(
            "This interview date is currently full. plaease contact " +
              vm.organization.admin +
              " for help scheduling an interview."
          );
          vm.loading = false;
          console.log(e);
        });
    ...
    checkCandidate() {
      axios
        .post(API_URL + "/candidate/get/" + this.org + "/" + this.uuid)
        .then(({ data }) => {
          console.log(data);
          this.candidate = data;
    ...
    loadDashboard() {
      ...
      const org = this.org;
      axios
        .get(API_URL + "/dashboard/admin/" + org)
        .then(({ data }) => {

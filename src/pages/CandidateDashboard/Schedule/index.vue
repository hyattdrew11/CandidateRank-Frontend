  ...
  <div>
      ...
      <table class="table table-bordered">
        <thead>
          <tr>
            ...
            <th scope="col" v-for="(x, index) in times" v-bind:key="index">
              {{ x }}
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(x, index) in currentDay.rooms" v-bind:key="index">
            <th scope="row" tyle="width: 150px;">
              ...
              <p
                ...
                class="tx-12 mb-0"
              >
                <i class="fa fa-user-circle mr-1 ml-2"></i>{{ e.email }}
              </p>
            </th>
            <td v-for="(t, index) in times" v-bind:key="index">
              <div
                v-if="
                  currentDay.events.find((el) => el.time === t && el.room === x)
                "
                :set="
                  (event = currentDay.events.find(
                    (el) => el.time === t && el.room === x
                  ))
                "
              >
                <div class="event" :style="{ 'background-color': event.color }">
                  ...
                  <img
                    v-if="event.candidate.photo != 'null'"
                    :src="event.candidate.photo"
    ...
    <b-container fluid>
      <div class="candidate-block-container">
        <div v-for="(x, index) in candidates" v-bind:key="index">
...
export default {
  ...
  methods: {
    ...
    getRandomColor() {
      let i =
        this.interviewTimes.interviewLength * this.currentDay.rooms.length;
    ...
    timeRange() {
      ...
      const minutes = Array.from(
        range.by("minutes", {
          excludeEnd: false,
          step: this.currentDay.interviewLength,
        })
      );

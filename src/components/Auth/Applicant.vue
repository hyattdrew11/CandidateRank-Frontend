  ...
  <div>
    ...
    <div :class="{fade : loading }">
      ...
      <div class="container-fluid">
        <b-row v-if="!showDashboard" id="login" align-v="center" align-h="center"  no-gutters>
          <b-col sm="12" md="4" lg="4" v-if="!authenticated">
            <b-card>
              <h5>Candidate Rank Applicants</h5>
              <span class="tx-12 wt-500">Please enter your email address to access your schedule.</span>
              <hr />
           ...
           <b-col sm="12" md="4" lg="4" v-if="authenticated && !showDashboard">
              <b-card>
                 ...
                 <b-form-select v-model="organization" @change="changeOrg(organization)">
                <option v-for="(x, index) in filteredOrgs" :key="index" :value="x">{{ x['name'].toUpperCase() }}</option>
             </b-form-select>
              </b-card>
           </b-col>
        </b-row>
         <b-row v-if="showDashboard" id="applicant-dashboard"  align-h="center"  no-gutters>
          <b-col sm="12" md="12" lg="12">
             <b-card>
               <h5>
                <span class="fa-stack fa-lg pointer themeGreen" @click="goBack()">
                  <i class="fa fa-circle fa-stack-2x"></i>
                  <i class="fa fa-arrow-left fa-stack-1x fa-inverse"></i>
                </span>
                Back
                <b-button @click="chooseNewDate()" size="sm" class="pointer p-2 float-right" variant="primary">  
                    Reschedule
                </b-button>
              </h5>
              ...
              <div v-if="'interview-date' in me || dateFinalized" class="idinme">
                <h5>My Schedule</h5>
                <h6><strong>My Interview Day: 
                  <span>{{ me['interview-date'] }}</span>
                  <span v-if="!('interview-date' in me)">{{ candidateSlots[0].startDate }}</span>
                </strong></h6>
                <h4><strong style="color: red;">Your interview date has been selected.  Check back 24 hours prior to your interview date to view your interview schedule. If you need to reschedule please contact info@candidate-rank.com </strong></h4>
                 <a v-if="waitingRoom" 
                    :href="waitingRoom" 
                    target="_blank" 
              ...
              <div v-if="dateFinalized">
                ...
                <b-list-group>
                  <b-list-group-item v-for="(x, index) in  candidateSlots" class="">
                    ...
                    <span class="wt-500">Evaluator(s):</span>
                      <span v-for="(e, index) in x.evaluators" class="mb-0">
                       ...
                       <b-button v-if="x.zoom_link == 'Waiting'" size="sm" class="pointer p-2 float-right" variant="secondary" disabled>  
                        Zoom Pending
                       </b-button>
                        <b-button  @click="openInterview(x)" v-if="x.zoom_link != 'Waiting' && x.zoom_link != 'Not Set'" size="sm" class="pointer p-2 float-right" variant="primary">Start Zoom</b-button>
                  </b-list-group-item>
                </b-list-group>

              </div>

              <div v-if="(!('interview-date' in me) && !dateFinalized)  || reschedule">
                <b-row>
                  ...
                  <b-col md="4" v-for="(x, index) in orgDates" class="mb-2">
                        <b-card  header="">
                          {{ x.startDate }}
                        <b-button :disabled="datePicked || 'finalized' in x || x.numCandidates >= x.maxCandidates" @click="chooseDate(x)" size="sm" variant="success" class="float-right mr-2">Choose Date</b-button>
                        <p v-if="'finalized' in x" class="tx-10 wt-500 red upper">Schedule finalized</p>
                        <p v-if="'numCandidates' in x && 'maxCandidates' in x" class="tx-10 wt-500 red upper">
                          <span v-if="x.numCandidates >= x.maxCandidates">Date Full</span>
                        </p>
                    </b-card>
                  </b-col>
         ...
         <div v-if="candidate" class="tx-8" style="opacity: 0; width: 0px; height: 0px; display: inline-block;position: fixed;bottom: 0px; left: 0px; "><span>{{ candidateSlots }}</span></div>
      </div>
    </div>
  </div>
</template>
...
export default {
                ...
                for(r in rooms) {
                  ...
                  for(e in events) {
                    if ('candidate' in events[e]) {
                      if(events[e].candidate.email == this.email || events[e].candidate['E-Mail'] == this.email) {
                        ...
                        if('waitingRoom' in dates[x]) { this.waitingRoom = dates[x].waitingRoom }
                        slots.push(events[e])
                      }
                    }
                  }
  ...
  methods: {
    ...
    chooseDate(date) {
      ...
      axios.post( API_URL+'/dashboard/candidate-date/'+org, input )
       .then(({ data }) => { 
        ...
        .catch(function (e) { 
          alert("This interview date is currently full. please contact " + vm.organization.admin + " for help scheduling an interview. Or choose another date.")
          vm.loading = false
          vm.datePicked = false
          console.log(e) 
        })
    ...
    authenticate () {
      ...
      axios.post( API_URL+'/auth/applicant/getorgs', input)
      .then(({data}) => { 

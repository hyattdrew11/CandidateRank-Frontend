<template>
  <b-container id="faculty-dashboard" fluid class="">
    <b-breadcrumb id="faculty-breadcrumb">
      ...
      <b-breadcrumb-item v-if="showCandidates" :active="showCandidates">{{ currentTerm.year }} Candidates {{ currentTerm.department }} </b-breadcrumb-item>
      <b-breadcrumb-item v-if="showSchedule"   :active="showSchedule">Interview Schedule {{ currentTerm.department}} {{ currentTerm.year }}</b-breadcrumb-item>
  </b-breadcrumb>
   ...
   <!--  <span v-if="currentTerm" id="currentTermLabel">
    ...
    <div :class="{fade : loading }">
      <b-col sm="12" class="mt-3">
        <b-row>
          <p style="margin-top: 200px; text-align: center;"  v-if="organization.terms.length == 0">No Interview Terms</p>
          <b-col class="mt-2" v-if="organization.terms && showTerms" sm="12" md="4" v-for="(x, index) in sortedTerms">
              <b-card  class=""
              ...
              <hr />
                <b-button size="sm" class="action-btn float-right mr-2" @click="viewSchedule(x)" variant="outline-secondary">View Schedule</b-button>
                <b-button size="sm" class="action-btn float-right mr-2" @click="viewCandidates(x)" variant="outline-secondary">View Candidates</b-button>
              </b-card>
           </b-col>
          <b-col :class="{'d-none': !showCandidates}" sm="12" md="12">
            <Grid :candidates="candidates" :organization="organization" :user="user" :currentTerm="currentTerm"/>
          </b-col>
          <b-col :class="{'d-none': !showSchedule}" sm="12" md="12">
            <Schedule :candidates="candidates" :organization="organization" :user="user" :currentTerm="currentTerm"/>
          </b-col>
        </b-row>
      </b-col>
    </div>
